...
- 🧱 **General & layout** — `button`, `segmented`, a two-state `toggle`, `divider`, `space`, `flex`, 24-column `grid` (`row`/`col`), a page-scaffold `layout` (`header`/`sider`/`content`/`footer`), and a draggable `splitter`.
